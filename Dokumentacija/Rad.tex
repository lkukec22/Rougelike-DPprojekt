...
Gramatika je definirana nizom pravila prepisivanja oblika \mintinline{text}{weighted_rule(ParentType, ChildType, Weight)}. Težina (weight) određuje relativnu vjerojatnost odabira tog tipa sobe. Primjer pravila iz koda:
\begin{listing}
\begin{minted}{prolog}
...
weighted_rule(treasure, boss, 60).   % Treasure vodi do bossa
weighted_rule(boss, _, 0).           % Boss je terminalni čvor
\end{minted}
\caption{Probabilistička pravila gramatike s težinama}
\label{lst:grammar_rules}
\end{listing}
Ova pravila, prikazana u ispisu \ref{lst:grammar_rules}, određuju koje nove sobe (čvorove) možemo stvoriti iz postojeće sobe određenog tipa. Veća težina znači veću vjerojatnost odabira - npr. shop ima samo 10\% šanse, dok combat ima 60\%.
...
\section{Prednosti}
\begin{itemize}
    \item \textbf{Odvajanje strukture od algoritma}: Promjenom samo \mintinline{text}{weighted_rule} predikata možemo drastično promijeniti topologiju tamnice (linearni hodnici vs. razgranati labirinti).
